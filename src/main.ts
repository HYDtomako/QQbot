...
import { matchMessage, type TriggeredMessage, type ImageRef, type FileRef } from "./router.ts";
...
async function handleTrigger(trigger: TriggeredMessage): Promise<void> {
  ...
  const task = prev.catch(() => undefined).then(async () => {
    ...
    const { images, files } = await collectAttachments(trigger);
    const { paths: filePaths, rejected } = files.length ? await materializeFiles(files) : { paths: [], rejected: [] };
    ...
    const imagePaths = images.length ? await materializeImages(images) : [];
...
// ── 图片附件：把 QQ 消息里的图片落到沙箱，供 pi 以 @路径 读取（多模态）──
const attachDir = path.join(sandboxDir, "attachments");
...
async function materializeImages(images: ImageRef[]): Promise<string[]> {
  ...
  for (const [i, img] of images.entries()) {
    const save = (buf: Buffer): string => {
      const p = path.join(attachDir, `img-${Date.now()}-${i}.${sniffExt(buf)}`);
      writeFileSync(p, buf);
      return p;
    };
...
/** 汇总消息里的图片：当前消息的 image 段 + 引用消息里的 image 段。 */
/** 汇总附件：当前消息的图片/文件 + 引用消息里的图片/文件。 */
async function collectAttachments(trigger: TriggeredMessage): Promise<{ images: ImageRef[]; files: FileRef[] }> {
    ...
    for (const s of segs) {
      if (s.type === "image") {
        ...
        files.push({
          fileId: typeof s.data?.file_id === "string" ? s.data.file_id : undefined,
          name:
            typeof s.data?.file_name === "string"
              ? s.data.file_name
...
async function materializeFiles(files: FileRef[]): Promise<{ paths: string[]; rejected: string[] }> {
  ...
  for (const f of files) {
    ...
    if (!buf) {
      rejected.push(`${name}（下载失败）`);
      log(`[file] ${name} 获取失败（fileId=${f.fileId ?? "-"} url=${f.url ? "有" : "无"}）`);
      continue;
    }
    if (buf.length > MAX_TEXT_FILE_BYTES) {
      rejected.push(`${name}（过大 ${Math.round(buf.length / 1024)}KB，上限 512KB）`);
      continue;
    }
    const safe = name.replace(/[\\/:*?"<>|\s]+/g, "_").slice(0, 60);
    const p = path.join(attachDir, `${Date.now()}-${safe}`);
    writeFileSync(p, buf);
    paths.push(p);
    log(`[file] ${name} 已落地（${Math.round(buf.length / 1024)}KB）`);
  }
...
setInterval(cleanupAttachments, 15 * 60_000).unref();
