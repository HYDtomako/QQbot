...
按顺序双击两个脚本，各开一个常驻窗口（用完直接关窗口即停止）：

1. **先启动 NapCat**：双击 `start-napcat.bat`
   - 自动用机器人号快速登录（脚本里配置），登录态失效时会回退二维码（见窗口内提示，扫码即可）。
   - 若电脑上的官方 QQ 正登录着机器人号，会登录失败——先退出官方 QQ 再启动（手机 QQ 不受影响）。
2. **再启动桥接服务**：双击 `start-bot.bat`

停止：关闭两个窗口，或直接关闭电脑（下次再双击启动）。
